...
def run_continuous(args, make_archive_fn):
    ...
    def worker(worker_id):
        ...
        try:
            while (time.time() - start) < duration_sec and (
                args.archives_count == 0 or counters["sent"] < args.archives_count
            ):
...
def run_burst(args, make_archive_fn):
    ...
    for cycle in range(num_cycles):
        ...
        def burst_worker(worker_id):
            tmpdir = tempfile.mkdtemp(prefix=f"send_archives_w{worker_id}_")
            try:
                while (time.time() - burst_start) < burst_sec and (
                    args.archives_count == 0
                    or (total_sent + counters["sent"]) < args.archives_count
                ):
                    cluster_id = str(uuid.uuid4())
                    is_bad = args.bad_ratio > 0 and random.random() < args.bad_ratio
                    path = None
        ...
        print(
            f"  Cycle {cycle + 1} done: {counters['sent']} archives ({counters['bad']} bad)"
        )
...
def main():
    ...
    if args.max_exceptions_archive:
        ...
        if not os.path.isfile(source_tar):
            ...
            sys.exit(1)
        make_fn = lambda cid, tmpdir: make_max_exceptions_archive(
            cid, tmpdir, source_tar
        )
